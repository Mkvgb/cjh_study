# -*- encoding: utf-8 -*-
"""
@File       :   etl_ele_fence_realtime.py
@Contact    :   suntang.com
@Modify Time:   2020/9/3 15:43
@Author     :   cjh
@Version    :   1.0
@Desciption :   None
"""
...
class AnalysisEleFence(object):
    def __init__(self, spark):
        ...
        self.PGSQL_TABLE_TRACK = "zhaoqing_duanzhou_db.track_copy2"
    ...
    def etl_real_time(self, file_path, precision_span, timestack_span):
        ...
        # df_ele_fence = self.spark.read.jdbc(url="jdbc:postgresql://192.168.1.99:6543/postgres", table="analysis_etl_gd_ele_fence.test_data_copy2", properties=self.PGSQL_PROPERTIES)
        ...
        etl_job.update_track_talbe(df_track_output)
    ...
    def etl_2_track(self, df_ele_fence, precision_span, timestack_span):
        ...
        # ele[mobile_mac, netbar_wacode, time_on, max_count]
    ...
    def union_track_2_hdfs(self, df_ele_fence, timestack_span):
        ...
        df_ele_fence.write.csv('hdfs://192.168.7.150:8020/test/cjh/csv/a', header=True, mode='overwrite')
    ...
    def standardize_etl_data(self, df_track_source):
        ...
        df_track_tmp = df_track_tmp.withColumnRenamed("mobile_mac", "probe_data").withColumn("create_time", functions.lit(int(time.time() * 1000)))
        ...
        if client.exists(self.HDFS_ST_DEVICE_SUB):
            ...
            func_text2long = udf(UdfUtils().udf_text2long, LongType())
            # df_max_dev = df_max_dev.withColumn("device_mac", func_etl("device_mac"))
            # df_max_dev = df_max_dev.withColumn("last_update_time", func_text2long("last_update_time"))
        ...
        df_track_output = df_track_output.withColumn("device_type", df_track_output.device_type.astype("long")).withColumnRenamed("flag", "flag1")
        func_null_to_zero = udf(UdfUtils().udf_null_to_zero, StringType())
        df_track_output = df_track_output.withColumn("flag", func_null_to_zero("flag1")).drop("flag1")

        df_track_output_2_hdfs = df_track_output.drop("flag")
        # 至此将所有更新后的track的数据都转为track表中的格式
        # [netbar_wacode, probe_time, probe_data, probe_type, create_time, datasource_table_name, longitude, latitude, device_type, probe_device_mac]
    ...
    def update_track_talbe(self, df_track_output):
        ...
        if client.exists(self.HDFS_ST_TRACK_INFO_SUB):
            ...
            df_track_old = self.spark.read.jdbc(url=self.PGSQL_URL_analysis, table=self.PGSQL_TABLE_TRACK, properties=self.PGSQL_PROPERTIES).limit(0).withColumn("flag", functions.lit(0))
        ...
        df_track_append.drop("flag").write.mode(saveMode='append').jdbc(url=self.PGSQL_URL_analysis, table=self.PGSQL_TABLE_TRACK, properties=self.PGSQL_PROPERTIES)
...
if __name__ == '__main__':
    ...
    file = ''
    AnalysisEleFence(spark).etl_real_time(file ,60 , 600)
    # if len(sys.argv) > 1:
