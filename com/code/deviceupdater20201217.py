# -*- encoding: utf-8 -*-
"""
@File       :   deviceupdater20201217.py    
@Contact    :   ggsddu.com
@Modify Time:   2020/12/17 11:59
@Author     :   cjh
@Version    :   1.0
@Description :   None
"""
# -*- encoding: utf-8 -*-
"""
@File       :   bak_deviceupdater.py
@Contact    :   ggsddu.com
@Modify Time:   2020/12/1 15:20
@Author     :   cjh
@Version    :   1.0
@Description :   None
"""
...
wifi_fields_change = {"collect_time": "time_on"}
audit_fields_change = {"collect_time": "time_on"}
im_fields_change = {"collect_time": "time_on"}
pgsql_properties = {'user': 'postgres', 'password': 'postgres'}
...
FILE_PATH_LIST = [
        "/weihai/src/audit_type",
        "/weihai/src/probe_type",
        "/weihai/src/wifi_type",
        "/weihai/src/im_type",
    ]
...
class UpdateTimeEleFence(object):
    ...
    def updatetimer_func(self, project_name=''):
        ...
        for i in range(len(file_path_list)):
            if self.HDFS_CLIENT.exists(file_path_list[i]):
                ...
                file_path_list_tmp = [x.replace("_combine", "") for x in file_path_list_tmp]
                if len(file_path_list_tmp) == 0:
                    print(file_path_list[i] + " is empty!")
                    continue
                file_path = file_path_list[i] + '/' + str(sorted([int(j) for j in file_path_list_tmp])[len(file_path_list_tmp) - 2])
                self.device_place_info_2hdfs_real_time(file_path)
        self.spark.stop()
    ...
    def device_place_info_2hdfs_real_time(self, file_path):
        ...
        print(file_path)
        if self.HDFS_CLIENT.exists(file_path + '_combine'):
            df_ef = self.spark.read.parquet(file_path + '_combine')
        elif self.HDFS_CLIENT.exists(file_path):
            df_ef = self.spark.read.parquet(file_path)
        else:
            return

        df_ef = self.prepare_ele_fence(df_ef, file_path)
        if type(df_ef) == int:
            print("table_name must be probe_type, wifi_type, audit_type, im_type!")
        df_dev_new = self.update_time_dev(df_ef, df_dev_old)
        self.update_time_pla_new(df_dev_new, df_dev_old,  df_pla_old)
    ...
    def update_time_dev(self, df_ef, df_dev_old):
        ...
        print(df)
        self.update_table(df, 'gd_device')
...
def append_pgsql(df, table, schema):
    engine = create_engine("postgresql://postgres:postgres@192.168.1.99:6543/postgres")
    sio = StringIO()
