...
RABBIT_MQ_SCHEDULE_ROUTE_KEY = 'schedule'
RABBIT_MQ_SCHEDULE_QUEUE = f'{RABBIT_MQ_PREFIX}_scheduler'
...
if not os.path.exists(log_path):
    os.mkdir(log_path)
logger.add(f'{log_path}/info.log', format="{time:YYYY-MM-DD HH:mm:ss} | {level} | {message}", retention="10 days",
           encoding='utf-8', level='INFO')
logger.add(f'{log_path}/warn.log', format="{time:YYYY-MM-DD HH:mm:ss} | {level} | {message}", retention="10 days",
           encoding='utf-8', level='WARNING')
logger.add(f'{log_path}/error.log', format="{time:YYYY-MM-DD HH:mm:ss} | {level} | {message}", retention="10 days",
           encoding='utf-8', level='ERROR', backtrace=True, diagnose=True)
...
class AnalysisEleFence(object):
    ...
    def etl_real_time(self, table_name, df_ef, precision_span, time_stack_span):
        ...
        attr_list = ['14-3C-C3-3A-90-43', '6E-E5-F7-1B-02-40', '58-E2-8F-CA-65-E3']
        ...
        df_ef = pd.read_parquet('../ggsddu/sparkcjh/data/probe_type_test.parquet')
        df_ef = etl_job.prepare_data(df_ef, attr_list, table_name)
        if type(df_ef) == int:
            return 0
    ...
    def union_track_2_hdfs(self, df_ef, time_stack_span):
        ...
        df_track = df_ef[["probe_mac", "place_code", "time_start", "time_end"]]
    ...
    def update_track_table(self, df_track_output, df_ef_dev):
        ...
        if self.hdfs_opr.check_path_is_exist(self.HDFS_ST_TRACK_INFO_SUB):
            try:
                df_track_old = self.hdfs_opr.read_csv_to_df(self.HDFS_ST_TRACK_INFO_SUB)
...
AnalysisEleFence().etl_real_time(table_name='probe_type', df_ef=1, precision_span=60, time_stack_span=600)
