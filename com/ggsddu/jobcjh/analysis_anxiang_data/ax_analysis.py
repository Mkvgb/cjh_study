...
class Analysis_anxiang_data(object):
    ...
    def spc_mobile_track(self, mobile_mac_list):
        ...
        # df = spark.read.csv('hdfs://192.168.7.150:8020/test/cjh/csv/co_ap_list_1588528801.csv', schema=sch)
        df = spark.read.csv('hdfs://192.168.7.150:8020/test/cjh/csv/co_ele_fence_1590170403.csv', schema=sch)
...
if __name__ == '__main__':
    ...
    df = df_ap[df_ap["ap_mac"] == 128080832925464]
    df.write.csv("hdfs://192.168.7.150:8020/test/cjh/csv/ap_test.csv", header=True)

    # df = spark.read.jdbc(url="jdbc:mysql://192.168.1.99:3306/test", table="a_mac_perpla", properties={'user': 'root', 'password': 'root123'})

    # lt1 = [28864944967802, 141352165890711, 88029466511761, 141352165895549, 123581278021554, 141352165895523, 26665921712232]

    lt2 = [1101097074682, 1101088686331, 1652522221568, 56294136348923, 1101088686102, 56294136348674, 56294136348673]

    # Analysis_anxiang_data(spark).spc_mobile_track(lt2)

    # Analysis_anxiang_data(spark).plot_spe_mobile_mac_timetrack(lt2)
    ...
    #
    # df_ele = df_ele[df_ele["mobile_mac"] == 1101097074682]
    #
    # df_ele.write.csv('hdfs://192.168.7.150:8020/test/cjh/csv/ele_test.csv', header=True)
