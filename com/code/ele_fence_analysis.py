...
class AnalysisEleFence(object):
    ...
    def etl(self, source_path, target_path, filename):
        """对所有的mobile_mac进行清洗，并入库"""
        df_ele_fence = self.spark.read.parquet(source_path + filename)
...
if __name__ == '__main__':
    ...
    # AnalysisEleFence(spark).etl(source_path, target_path, filename)
    df = spark.read.parquet('hdfs://192.168.7.150:8020/anxiang/co_ele_fence_compress/1587837602.csv')
    df.show()
    print(df.count())
    spark.stop()
